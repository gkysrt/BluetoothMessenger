...
class ListViewDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        iconRect = QtCore.QRect(option.rect.topLeft() + QtCore.QPoint(self.marginLeft(), self.marginTop()),
                                QtCore.QPoint(option.rect.height() - self.__bottomMargin - self.__topMargin,
                                              option.rect.y() + option.rect.height() - self.__bottomMargin))
        connectionRect = QtCore.QRect(
            QtCore.QPoint(option.rect.width() - option.rect.height() / 5 * 3 - self.marginRight(),
                          option.rect.y() + option.rect.height() / 5),
            QtCore.QPoint(option.rect.width() - self.marginRight(),
                          option.rect.y() + option.rect.height() - option.rect.height() / 5))
        nameRect = QtCore.QRect(QtCore.QPoint(iconRect.topRight().x() + self.spacing(), iconRect.topRight().y() + 2),
                                QtCore.QPoint(
                                    option.rect.width() - connectionRect.width() - self.spacing() - self.marginRight(),
                                    option.rect.y() + option.rect.height() / 2 - self.spacing() / 2))
        macRect = QtCore.QRect(
            QtCore.QPoint(iconRect.topRight().x() + self.spacing(), nameRect.bottomLeft().y() + self.spacing() - 3),
            QtCore.QPoint(option.rect.width() - connectionRect.width() - self.spacing() - self.marginRight(),
                          option.rect.y() + option.rect.height() - self.marginBottom() - 5))
        ...
        if hCoefficient > wCoefficient:
            ...
            iconBoundingRect = QtCore.QRect((iconRect.width() - width) / 2 + iconRect.x(), iconRect.y(), width,
                                            iconRect.height())

        elif hCoefficient < 1 and wCoefficient < 1:
            iconBoundingRect = QtCore.QRect((iconRect.width() - iconToDraw.rect().width()) / 2 + iconRect.x(),
                                            (iconRect.height() - iconToDraw.rect().height()) / 2 + iconRect.y(),
                                            iconToDraw.rect().width(), iconToDraw.rect().height())

        else:
            height = iconToDraw.height() * iconRect.width() / iconToDraw.width()
            iconBoundingRect = QtCore.QRect(iconRect.x(), (iconRect.height() - height) / 2 + iconRect.y(),
                                            iconRect.width(), height)
        ...
        if shouldDrawLine:
            ...
            painter.drawLine(QtCore.QPoint(option.rect.width() / 6, option.rect.height() - 1 + option.rect.y()),
                             QtCore.QPoint(option.rect.width() * 5 / 6, option.rect.height() - 1 + option.rect.y()))
